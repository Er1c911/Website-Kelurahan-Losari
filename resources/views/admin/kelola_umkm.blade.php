                        ...
                        <form action="{{ route('admin.kelola-umkm.update', $item) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                            ...
                            @if ($item->photo_data || $item->photo_path)
                                <div>
                                    ...
                                    <img src="{{ $item->photo_data ?: \Illuminate\Support\Facades\Storage::disk(config('filesystems.media', 'public'))->url($item->photo_path) }}" alt="{{ $item->name }}" class="w-full max-w-md rounded-lg border border-gray-200 object-cover mb-2">
                                    <form action="{{ route('admin.kelola-umkm.photo.destroy', $item) }}" method="POST" onsubmit="return confirm('Hapus foto UMKM ini?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="inline-flex items-center gap-1 bg-red-100 hover:bg-red-200 text-red-700 text-xs font-semibold px-3 py-1.5 rounded-lg transition">
